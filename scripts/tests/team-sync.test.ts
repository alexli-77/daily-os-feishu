...
async function main(): Promise<void> {
  ...
  async function testUiServer(): Promise<void> {
    ...
    try {
      ...
      check('team status is ready when signed in with a team', state?.team?.status === 'ready', String(state?.team?.status));
      check('team members carry cached cycles for the switcher', (state?.team?.members?.[0]?.cycles || []).length > 0, JSON.stringify(state?.team?.members?.[0]?.cycles?.length));
      check('the teammate cycle content reaches the page', JSON.stringify(state?.team?.members?.[0]?.cycles || []).includes('企鹅在控制台里的要务'));
      ...
      check('signed out, the team block says so', signedOutState?.team?.status === 'signed_out', String(signedOutState?.team?.status));
      check('signed out, no teammate cycles are offered', (signedOutState?.team?.members || []).length === 0);
      ...
      check('with no team, the team block says so', noTeamState?.team?.status === 'no_team', String(noTeamState?.team?.status));
      check('with no team, the reason is explained', String(noTeamState?.team?.reason || '').includes('团队'), String(noTeamState?.team?.reason));
  ...
  async function testConsoleRendering(): Promise<void> {
    ...
    page.setState({ cycles: myCycles, team: teamState });
    page.render();
    ...
    page.setState({ cycles: myCycles, team: { ...teamState, members: [{ ...teamState.members[0], cycles: [] }] } });
    page.render();
    page.click('cycle-members', MATE_ID);
    ...
    page.setState({ cycles: myCycles, team: { ...teamState, status: 'signed_out', reason: '尚未登录团队账号，同步已暂停，本地读写不受影响。', members: [] } });
    page.render();
    ...
    // No team yet.
    page.setState({ cycles: myCycles, team: { ...teamState, status: 'no_team', reason: '账号还没有加入团队，暂时看不到队友的周期，本地读写不受影响。', members: [] } });
    page.render();
    ...
    page.setState({ cycles: myCycles, team: { ...teamState, lastError: 'fetch failed' } });
    page.render();
...
async function loadCyclesPage() {
  ...
  const factory = new Function(
    ...
    `${JS}
     return {
       renderCycles: () => renderCycles(state && state.cycles, state && state.team),
       selectCycleOwner,
